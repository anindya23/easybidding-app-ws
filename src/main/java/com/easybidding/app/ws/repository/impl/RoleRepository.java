...
public interface RoleRepository extends BaseRepository<RoleEntity, String> {
	...
	@Query("SELECT r FROM RoleEntity r JOIN r.account a WHERE a.id = :accountId AND r.roleCode = :roleCode AND r.status = 'ACTIVE'")
	public RoleEntity findRoleByAccount(@Param("accountId") String accountId, @Param("roleCode") String roleCode);
	
	@Query("SELECT r FROM RoleEntity r JOIN r.account a WHERE a.id = :accountId AND r.status = 'ACTIVE'")
	...
	@Query("SELECT r FROM RoleEntity r JOIN r.account a WHERE a.id IS NULL AND r.roleCode = :roleCode AND r.status = 'ACTIVE'")
